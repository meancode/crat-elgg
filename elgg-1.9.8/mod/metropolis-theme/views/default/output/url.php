...
if ($url) {
	...
	if (elgg_extract('span_before', $vars, false)) {
	    $iconclassbefore = elgg_extract('icon_class_before', $vars, '');
	    $iconbefore = elgg_extract('link_icon_before', $vars, '');
		$spanclassbefore = '<span class="'.$iconclassbefore.' '. $iconbefore.'  onleft"></span>';
	}
    if (elgg_extract('span_after', $vars, false)) {
	    $iconclassafter = elgg_extract('icon_class_after', $vars, '');
	    $iconafter = elgg_extract('link_icon_after', $vars, '');
		$spanclassafter = '<span class="'.$iconclassafter.' '. $iconafter.'  onright"></span>';
	}
...
echo "<a $attributes>$spanclassbefore $text $spanclassafter</a>";
